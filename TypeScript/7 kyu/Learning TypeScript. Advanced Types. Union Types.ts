...
// Union types are closely related to intersection types, but they are 
// used very differently.Occasionally, you’ll run into a library that 
...
// The problem with padLeft is that its padding parameter is typed as 
// any.That means that we can call it with an argument that’s neither a 
// number nor a string, but TypeScript will be okay with it.
...
// explicit, it’s also a little bit overkill.One of the nice things about the 
// original version of padLeft was that we were able to just pass in 
...
// I.e.if we have a value that has a union type, we can only access 
// members that are common to all types in the union.If a value has the 
// type A | B, we only know for certain that it has members that both 
// A and B have.

// Your task is to create function join(tokens: string | string[], glue?: string): string which 
// can accept both string and string[] and return one string in which 
// tokens are concatenated with the glue.
...
export function join(tokens: string | string[], glue?: string): string {
    ...
    if (Array.isArray(tokens) && tokens.every(item => typeof item === 'string')) return tokens.join(glue)
    return "invalid"
}