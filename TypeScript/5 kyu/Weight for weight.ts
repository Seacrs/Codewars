...
// a "weight" to numbers. The weight of a number will be from now on 
...
//     whitespaces and more than a unique whitespace between two consecutive numbers

//     For C: The result is freed.

export function orderWeight(strng: string): string {
    return strng.split(" ")
                .map((el):[string, number] => [el, el.split("").reduce((acc, curV)=>acc + +curV, 0)])
                .sort(([c, a], [k, b])=> a === b ? c.localeCompare(k): a - b)
                .map(([word]) => word)
                .join(" ")
}