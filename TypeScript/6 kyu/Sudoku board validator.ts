...
// valid Sudoku solution, or false otherwise. The cells of the input Sudoku 
...
//     Rows, columns and blocks (3x3 small squares) must contain each 
//     number from range 1-9 exactly once.
...
export const validateGroups = (groups: number[][]): boolean =>{
    if(groups.length !== 9) return false;
    if(groups.some((arr: number[]) => arr.length !== 9)) return false;
    if(!groups.every((arr: number[])=> new Set(arr).size === 9 && arr.every(el => el >= 1 && el <= 9))) return false;
    return true;
}

export function validateSudoku(board: Board): boolean {
    if(!validateGroups(board)) return false;
    ...
    if(!validateGroups(columns)) return false;

    const boxes: number[][] = [];

    for(let i = 0; i < 9; i+=3){
        for(let j = 0; j < 9; j+=3){
            let row1 = board[i].slice(j, j+3);
            let row2 = board[i+1].slice(j, j+3);
            let row3 = board[i+2].slice(j, j+3);
            boxes.push([...row1, ...row2, ...row3]);
        }
    }

    if(!validateGroups(boxes)) return false;
