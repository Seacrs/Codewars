...
// Blaise de Vigenère, who had developed a stronger autokey cipher 
...
// places; for example, in a Caesar cipher of shift 3, A would become D, B would become 
// E, Y would become B and so on. The Vigenère cipher consists of several Caesar ciphers in 
...
// some implementations repeat the key over characters only if they are part of the alphabet -- this 
...
function VigenèreCipher(key, abc) {
    ...
    //...
        let alphabet = [...abc];
        let shift = key.repeat(str.length);
        let repeated = shift.slice(0, str.length);
        return [...str].map((el,index) =>{
            if(el === " ") return el;
            const pos1 = alphabet.indexOf(el);
            if(pos1 === -1) return el;
            const l = repeated[index];
            const pos2 = alphabet.indexOf(l);
            let pos3 = pos1 + pos2;
            if(pos3 >= alphabet.length) pos3 = pos3 % alphabet.length;
            return alphabet[pos3];
        }).join("")
    };
    ...
    //...
        ...
        return [...str].map((el,index)=>{
            ...
            const l = repeated[index];
            const pos2 = alphabet.indexOf(l);
            let pos3 = (pos1 - pos2 + alphabet.length) % alphabet.length;
            return alphabet[pos3];
        }).join("");
    };
}