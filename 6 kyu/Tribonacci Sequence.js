...
// the next. And, worse part of it, regrettably I won't get to hear non-
// native Italian speakers trying to pronounce it :(
...
// get the same sequence shifted by 2 places, but that is not the case and 
// we would get:
...
// Signature will always contain 3 numbers; n will always be a non-
// negative number; if n == 0, then return an empty array (except in C 
...
function tribonacci(signature,n){
  ...
  let index =0;
  while (arr.length < n){
    let sum = arr.slice(index,index+3).reduce((accumulator,currentVal)=>accumulator+ currentVal,0);
    arr.push(sum);
    index++;
  }
  return arr;
