...
// with a signature of 5 elements and each following element is the sum 
// of the 5 previous, and so on.
...
function Xbonacci(signature,n){
  ...
  while (arr.length < n){
    let sum = arr.slice(-index).reduce((accumulator,currentVal)=>accumulator+ currentVal);
    arr.push(sum);
  }
  return arr;
}