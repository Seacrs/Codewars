// Alex is a devoted fan of snooker Masters and in particular, he recorded 
// results of all matches. Help Alex to know the score of matches.
...
function frame(score){
    const frame = score.split(";");
    let count1 = 0;
    let count2 = 0;
    frame.forEach(el=>{
        const parts = el.split(/[-(]/).filter(b => !/[)]/.test(b)).map(Number);
        const [p1, p2] = parts;
        if(p1 > p2) count1++;
        else count2++;
    });
