...
// For example, encode("hello") would return "h2ll4". There is no 
// need to worry about uppercase vowels in this kata.
...
function encode(string) {
    let vowels = 'aeiou';
    return string.split("").map(el => vowels.includes(el) ? vowels.indexOf(el) + 1 : el).join("");
}

function decode(string) {
    let vowels = 'aeiou';
    return string.split("").map(el => '12345'.includes(el) ? vowels.charAt(el - 1) : el).join("");
}