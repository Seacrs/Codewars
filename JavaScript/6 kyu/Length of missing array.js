...
// I have created other katas. Have a look if you like coding and 
// challenges.

function getLengthOfMissingArray(arrayOfArrays) {
    if(!arrayOfArrays || arrayOfArrays.length === 0) return 0;
    if(!arrayOfArrays.every(arr=> arr && arr.length !== 0)) return 0;

    const arr = arrayOfArrays.map(arr => arr.length).sort((a,b)=>a-b);
    let sum = 0;
    ...
    return sum - arr.reduce((acc, curV)=> acc + curV);
}