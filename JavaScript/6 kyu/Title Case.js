...
// be given as a string with each word separated by a space. Your 
...
const titleCase = (title, minorWords) =>{
    ...
    let newArr = minorWords.split(' ').map(x=>x.toLowerCase());
    return title.toLowerCase().split(' ').map((word, i) => newArr.includes(word) && i !== 0 ? word : word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
}