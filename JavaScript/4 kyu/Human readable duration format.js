...
// 1 year, etc. In general, a positive integer and one of the valid units of 
// time, separated by a space. The unit of time is used in plural if the
...
// significant one. Therefore, 1 second and 1 year is not correct, but 
...
function formatDuration (seconds) {
    ...
    if(y) arr.push(`${y} year${y > 1 ? 's': ''}`);
    if(d) arr.push(`${d} day${d > 1 ? 's': ''}`);
    if(h) arr.push(`${h} hour${h > 1 ? 's': ''}`);
    if(m) arr.push(`${m} minute${m > 1 ? 's': ''}`);
    if(s) arr.push(`${s} second${s > 1 ? 's': ''}`);
