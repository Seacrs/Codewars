// For this exercise you will be strengthening your page-fu mastery.You 
// will complete the PaginationHelper class, which is a utility class 
...
class PaginationHelper {
    constructor(collection = [], itemsPerPage) {
        ...
        for (let i = 0; i < collection.length; i += itemsPerPage) {
            this.arr.push(collection.slice(i, i + itemsPerPage))
        }
    }
    itemCount() {
    ...
    pageItemCount(pageIndex) {
        if (pageIndex < 0 || pageIndex >= this.arr.length) return -1
        return this.arr[pageIndex].length
    }
    pageIndex(itemIndex) {
